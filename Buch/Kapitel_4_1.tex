...
Zunächst muss einmal abgegrenzt werden, wie sich reale Rigiditäten von nominalen Rigiditäten abgrenzen. Nominale Rigiditäten haben wir schon als "`Menu Costs"' kennengelernt. Reale Rigiditäten gehen einen Schritt weiter. Diese kann man so definieren, dass sich ein Preis selbst dann nicht ändert, wenn sich seine Bestimmungsfaktoren ändern. Also zum Beispiel wenn die Nachfrage nach Bier fällt, oder der Preis für Hopfen fällt, geht man davon aus, dass auch der Preis von Bier selbst fällt. Ist dies nicht der Fall spricht man von realen Rigiditäten. Wie sind reale Rigiditäten zu erklären? Ein ganzes Forschungsfeld (vgl. Kapitel \ref{drei Grunde}) wurde dazu entwickelt, teilweise schon vor den Überlegungen zur hier dargestellten Auswirkung von Rigiditäten. \textcite{RomerDavid1990} verbanden mit ihrer Arbeit das Forschungsfeld der Rigiditäten mit dem Forschungsfelder in Kapitel \ref{drei Grunde}. Mit ihren Überlegungen grenzten die beiden Autoren den Neu-Keynesianismus ein weiteres Mal entscheidend als eigene ökonomische Denkrichtung ab.
\textcite[S. 183]{RomerDavid1990} heben dabei gleich zu Beginn ihres Artikels hervor, "`dass \textit{reale} Rigiditäten nicht das gleiche sind wie \textit{nominale} Rigiditäten"'. Ende der 1980er Jahre gab es nämlich unterschiedslos viele Forschungsarbeiten zu Rigiditäten. 
Im nächsten Schritt erstellen die beiden ein interessantes aber komplexes Modell. Dessen Grundaussage lautet wie folgt: Erstens, nominale Rigiditäten ("`Menu Costs"') können realistischer Weise auch nur zu kleinen gesamtwirtschaftlichen Schwankungen führen. Zweitens, ausschließlich reale Rigiditäten können sogar überhaupt nicht existieren: Auf vollständigen Konkurrenzmärkten (dies entspricht einem Markt ohne jegliche nominale Rigidität), kommt es immer zur Anpassung an das Marktgleichgewicht. Würde man hier aufhören, wäre die Essenz: Rigiditäten spielen keine Rolle. Aber jetzt kommt der Clou aus \textcite{RomerDavid1990}: Gibt es nominale Rigiditäten, so werden diese durch reale Rigiditäten verstärkt und es kann zu großen Schwankungen im Gesamtoutput kommen. Diese wiederum rechtfertigen - wie schon im Paper von \textcite{Akerlof1985} - den Einsatz aktiver Wirtschaftspolitik.
